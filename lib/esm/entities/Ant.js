...
import { AntAction, AntState, CellStates, directions, LastChoice } from "../types";
...
export class Ant {
    ...
    score(currentAngle, world, tick) {
        ...
        nextAngles.forEach((angle, index) => {
            let score = { direction: angle, score: getScoreForDirection(angle, this.location, this.state, world, tick) };
            scores.push(score);
        });
        return scores;
    }

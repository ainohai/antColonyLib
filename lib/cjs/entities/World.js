...
class Cell {
    ...
    addPheremone(pheremoneType, startingStep, currentTick) {
        ...
        if (pheremoneType === types_1.PheremoneType.SUGAR) {
            let foodPheremoneAdd = (0, antConfig_1.antConfig)().antFoodPheremoneWeight * (1 - ((0, antConfig_1.antConfig)().antFoodPheremoneDecay * (currentTick - startingStep)));
            foodPheremoneAdd = foodPheremoneAdd > 0 ? foodPheremoneAdd : 0;
            this.foodPheremone = this.boundPheremone(0, (0, antConfig_1.antConfig)().maxPheremone, this.foodPheremone, foodPheremoneAdd);
        }
        if (pheremoneType === types_1.PheremoneType.HOME) {
            let homePheremoneAdd = (0, antConfig_1.antConfig)().antHomePheremoneWeight * (1 - ((0, antConfig_1.antConfig)().antHomePheremoneDecay * (currentTick - startingStep)));
            homePheremoneAdd = homePheremoneAdd > 0 ? homePheremoneAdd : 0;
            this.homePheremone = this.boundPheremone(0, (0, antConfig_1.antConfig)().maxPheremone, this.homePheremone, homePheremoneAdd);
        }
    }
}
exports.Cell = Cell;
    ...
    getFoodCoordinates() {
        ...
        if (this.foods.length < 12) {
            this.setFood(Math.floor(Math.random() * (0, antConfig_1.staticParameters)().COLUMNS), Math.floor(Math.random() *
                (0, antConfig_1.staticParameters)().ROWS), Math.floor(Math.random() * 1000));
        }
        return foods;
    }
    getHomeCoord() {
